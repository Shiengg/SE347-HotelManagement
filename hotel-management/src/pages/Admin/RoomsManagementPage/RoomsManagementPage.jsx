...
import { Button, Modal, Form, Input, Select, InputNumber, message, Space, Drawer, Carousel } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined, HomeOutlined, SortAscendingOutlined, SortDescendingOutlined, CheckCircleOutlined, CloseCircleOutlined, ToolOutlined, CalendarOutlined, DollarOutlined, ClockCircleOutlined, SettingOutlined, RightOutlined, LeftOutlined } from '@ant-design/icons';
...
const RoomImage = styled.div`
  ...
  background: ${props => props.imageUrl
    ...
    : 'linear-gradient(120deg, #1a3353, #264773)'};
  display: flex;
  align-items: center;
  justify-content: center;
  color: white;
...
const RoomsManagementPage = () => {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      // Xử lý ảnh mới và ảnh đã có
      if (values.images && values.images.length > 0) {
        for (let file of values.images) {
          if (file.originFileObj) {
            // Đây là file mới, chuyển thành base64
            const base64 = await getBase64(file.originFileObj);
            images.push(base64);
          } else if (file.url) {
            // Đây là ảnh đã có từ trước
            images.push({
              public_id: file.uid,
              url: file.url
            });
      ...
      // Nếu đang edit, tìm các ảnh đã bị xóa
      if (selectedRoom && selectedRoom.images) {
        const currentImageIds = values.images?.map(img => img.uid) || [];
        const deletedOnes = selectedRoom.images.filter(
          img => !currentImageIds.includes(img.public_id)
        );
        deletedImages.push(...deletedOnes);
      }
  ...
  // Thêm nút điều hướng cho carousel
                  ...
                  isEditing ? (
                    <FormWrapper>
                      ...
                      >
                        ...
                        <FormSection>
                          <div className="section-title">Room Images</div>
                          <Form.Item
                            name="images"
                            label="Room Images"
                    ...
                    <>
                      ...
                      <RoomImage imageUrl={selectedRoom.images?.[0]?.url}>
                        {!selectedRoom.images?.[0]?.url && <HomeOutlined />}
                      </RoomImage>
                        ...
                        {selectedRoom.images && selectedRoom.images.length > 0 ? (
                          <StyledCarousel {...carouselSettings}>
                            {selectedRoom.images.map((image, index) => (
                              <div key={index}>
                                <img src={image.url} alt={`Room ${selectedRoom.roomNumber} - ${index + 1}`} />
                              </div>
                            ))}
                          </StyledCarousel>
                        ) : (
